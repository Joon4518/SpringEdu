...
public class PageMaker { // 페이징처리 클래스
	...
	private int totalCount; // 총 게시글의 수 : 총 게시글의 수를 알아야 몇페이지가 나오는지 알 수 있다.
	...
	public void makePagein() {
		...
		// 질문. 내가 현재 13페이지를 보고 있다. 그렇다면 우측끝에 있는 마지막 페이지 번호는? 20
		// 7 / 10.0 -> 소수발생 시 올림 -> 1 * 10 -> 10
		// 13 / 10.0 -> 2 * 10 -> 20
		endPage = (int)(Math.ceil(cri.getPage() / (double)displayPageNum) * displayPageNum) ;
		
		// 2. 화면에 보여질 시작 페이지 번호
		// 현재 7페이지 -> 1 ~ 10   10(마지막) - 10(보여줄 페이지 개수) + 1
		// 현재 15페이지 -> 20(마지막) - 10(보여줄 페이지 개수) + 120 - 10 + 1
