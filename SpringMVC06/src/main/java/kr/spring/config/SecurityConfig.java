...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Bean // 우릭가 만들어놓은 MemberUserDetailsService 메묄로 올려 사용하겠다
			...
			.authorizeRequests()  // 요청에 따른 권한을 처리   
				.antMatchers("/") // 어떠한 경로로 왔을때 권한처리를 할 것인지
					...
					.formLogin() // 로그인 보안기능 추가
						...
						.loginProcessingUrl("/login.do") // 해당 url로 요청이 들어왔을 때 자체 Spring Security 자체 로그인 기능 수행.
					...
					/* 로그인 안하고 특정페이지에 접근하려고 할때 특정 url로 요청 */
	}
